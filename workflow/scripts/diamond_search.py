...
def main():
    logger = setup_logger("diamond_search")
    start_time = datetime.now()
    logger.info("=" * 60)
    logger.info("DIAMOND Search Started")
    logger.info("=" * 60)
    ...
    if not os.path.isfile(args.db):
        logger.error(f"DIAMOND database {args.db} not found")
        sys.exit(1)
    ...
    try:
        ...
        logger.error(f"Error getting diamond version: {e}")
        sys.exit(1)
    ...
    logger.info("=" * 60)
    logger.info("DIAMOND Search Completed Successfully")
    logger.info("=" * 60)
