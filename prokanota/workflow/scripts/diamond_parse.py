#!/usr/bin/env python3
"""
Parse DIAMOND tabular output and join with mapping file.
...
def filter_and_deduplicate(
    ...
    Best = highest score, lowest e-value, highest subject coverage, then accession.
    """
    return (
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parse DIAMOND results and join with mapping"
    )
    parser.add_argument(
        "--diamond-output", required=True, help="Path to DIAMOND tabular output file"
    )
    ...
    log_file_paths(
        logger,
        search_output=args.diamond_output,
    ...
    # Parse inputs
    logger.info("Parsing DIAMOND search results...")
    hits_df = parse_diamond_output(Path(args.diamond_output))
    ...
    logger.info("=" * 60)
    logger.info("DIAMOND Parse Completed Successfully")
    logger.info("=" * 60)
