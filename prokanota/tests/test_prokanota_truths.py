"""Tests for stable behavior defined by Prokanota."""
...
# ---
# 1. Protein and file utility contracts (feature_utils.py)
# ---

# These tests describe deliberate Prokanota behavior rather than a universal
# biochemical truth. A zero molecular weight is used as an explicit marker that
# a calculation could not be performed safely. Similarly, ensure_empty_file()
# is used when an optional output has no records: downstream Snakemake rules
# still require the declared file to exist, but it must contain no stale data.
...
def test_empty_sequence_molecular_weight():
    """
    Verify that an empty protein produces the same zero marker.
    There are no residues from which a meaningful molecular weight can be
    calculated.
    """
    assert feature_utils.protein_molecular_weight("") == 0


def test_ensure_empty_file_creates_parent_directories(tmp_path):
    """
    Verify that a rule can declare an empty output even when its output
    directory has not yet been created.
    """
    output_path = tmp_path / "nested" / "output.tsv"

    feature_utils.ensure_empty_file(str(output_path))

    assert output_path.exists()
    assert output_path.stat().st_size == 0


def test_ensure_empty_file_truncates_existing_content(tmp_path):
    """
    Verify that rerunning a no-result step cannot leave records from an older
    successful run in the declared output file.
    """
...
# ---
# 2. Sequence utility contracts (feature_utils.py)
# ---

# These tests specify Prokanota's handling of sequence representation and
# one-based inclusive coordinates. They are separated from the published
# complement rules because case preservation, empty-input handling, and
# coordinate slicing are software-interface decisions.
...
def test_reverse_complement_preserves_case():
    """
    Verify that reverse complementation preserves the input sequence's case.
    Sequence case is formatting, so the function must not silently rewrite it.
    """
...
def test_get_sequence_plus_strand():
    """
    Verify extraction with Prokanota's one-based, end-inclusive coordinates on
    the plus strand.
    """
    contig = "ATGCCGTA"
...
def test_get_sequence_minus_strand():
    """
    Verify that the same coordinate convention is used on the minus strand and
    that the extracted interval is reverse-complemented before it is returned.
    """
    contig = "ATGCCGTA"
